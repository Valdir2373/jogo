...
export function Telepathy({ userId, resumeRoomId, onBack, onGameChanged }: Props) {
  ...
  if (room.screen === 'lobby') return <GameLobby title="Telepatia" gameType="telepathy" error={room.error} onCreateRoom={room.createRoom} onBack={onBack} />
    ...
    <div className="flex flex-col items-center gap-4 w-full max-w-sm">
      ...
      <div className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-center">
        ...
        <div className="w-full bg-zinc-800 rounded-full h-1.5 mt-2">
          <div className="bg-primary h-1.5 rounded-full transition-all" style={{ width: `${((state?.score ?? 0) / (state?.target ?? 10)) * 100}%` }} />
        </div>
      </div>
