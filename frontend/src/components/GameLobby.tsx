        ...
        <div className="w-full flex flex-col gap-1.5">
          ...
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => setMaxPlayers(0)}
              className={`px-3 py-1.5 rounded-lg text-xs border transition-all ${maxPlayers === 0 ? 'border-primary bg-primary-bg text-primary' : 'border-zinc-700 text-zinc-400 hover:border-zinc-500'}`}>
              Padrão ({meta.maxPlayers})
            </button>
            {Array.from({ length: meta.maxPlayers - meta.minPlayers }, (_, i) => meta.minPlayers + i).map(n => (
              <button key={n} onClick={() => setMaxPlayers(n)}
                className={`px-3 py-1.5 rounded-lg text-xs border transition-all ${maxPlayers === n ? 'border-primary bg-primary-bg text-primary' : 'border-zinc-700 text-zinc-400 hover:border-zinc-500'}`}>
                {n}
              </button>
            ))}
          </div>
    ...
    <div className="w-full flex flex-col gap-2">
      <div className="flex justify-between items-center w-full">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 bg-zinc-900 border border-zinc-700 hover:border-primary rounded-lg text-xs text-zinc-400 hover:text-primary transition-all"
      ...
      {showPicker && (
        <div className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 flex flex-col gap-1.5">
            ...
            return (
              <button key={g.type} onClick={() => { onVoteGame(g.type); setShowPicker(false) }}
                className={[
                  ...
                  iV ? 'border-primary bg-primary-bg text-primary'
          ...
          {theirVote && !myVote && (
            <p className="text-xs text-primary text-center pt-1 animate-pulse">
...
export function RestartVoting({ iVoted, theyVoted, onVote, label = 'Jogar de novo' }: RestartVotingProps) {
  return (
    <div className="w-full flex flex-col gap-2 mt-2">
      ...
      {theyVoted && !iVoted && (
        <p className="text-center text-primary text-sm animate-pulse">
